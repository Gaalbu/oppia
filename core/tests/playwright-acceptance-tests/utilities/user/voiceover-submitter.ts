...
const commonModalConfirmButtonSelector = '.e2e-test-confirm-action-button';
...
export class VoiceoverSubmitter extends BaseUser {
  ...
  async deleteVoiceoverInCurrentCard(): Promise<void> {
    ...
    await this.expectElementToBeVisible(commonModalConfirmButtonSelector);
    await this.clickOnElementWithSelector(commonModalConfirmButtonSelector);
    await this.expectElementToBeVisible(
      commonModalConfirmButtonSelector,
      false
    );
  }
  ...
  /**
   * Checks that the preview audio bar shows the expected voiceover language.
   * @param language - The expected language and accent.
   */
  ...
  /**
   * Checks the visibility of the warning shown when a voiceover needs updating.
  ...
  /**
   * Checks that the upload dialog displays the expected error message.
